...
public class RemoveForbiddenWords {
  ...
  public static void fillAllWords() {
    allWords = new String[(int)Math.pow((double) FindForbiddenWords.baseToUse, (double) FindForbiddenWords.allowedWord.length())][2];
    for (int i = 0; i < allWords.length; i++) {
      ...
      allWords[i][0] = String.format("%0"+FindForbiddenWords.allowedWord.length()+ 'd', new
      BigInteger(Long.toString((long) i, FindForbiddenWords.baseToUse)));
